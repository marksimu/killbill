...
public class ProfilingContainerResponseFilter implements ContainerRequestFilter, ContainerResponseFilter {
    ...
    public void filter(final ContainerRequestContext request) throws IOException {
        ...
        final String profilingHeaderRequests = request.getHeaderString(PROFILING_HEADER_REQ);
        final String profilingHeaderRequest = (profilingHeaderRequests == null || profilingHeaderRequests.isEmpty()) ? null : profilingHeaderRequests;
        if (profilingHeaderRequest != null) {
            try {
                ...
                if (profilingData.getProfileFeature().isProfilingJAXRS()) {
                    profilingData.addStart(ProfilingFeatureType.JAXRS, request.getUriInfo().getPath());
                }
        ...
        try {
            ...
            if (rawData != null) {
                if (rawData.getProfileFeature().isProfilingJAXRS()) {
                    rawData.addEnd(ProfilingFeatureType.JAXRS, request.getUriInfo().getPath());
                }
                ...
                try {
                    ...
                    response.getHeaders().add(PROFILING_HEADER_RESP, value);
