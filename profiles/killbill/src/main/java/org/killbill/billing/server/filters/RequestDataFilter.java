...
public class RequestDataFilter implements ContainerRequestFilter, ContainerResponseFilter {
    ...
    public void filter(final ContainerRequestContext requestContext) throws IOException {
        final List<String> requestIdHeaderRequests = getRequestId(requestContext.getHeaders());
    ...
    private List<String> getRequestId(final MultivaluedMap<String, String> requestHeaders) {
        List<String> requestIds = requestHeaders.get(REQUEST_ID_HEADER);
        if (requestIds == null || requestIds.isEmpty()) {
            requestIds = requestHeaders.get(LEGACY_REQUEST_ID_HEADER);
        }
        return requestIds;
    }
    ...
    private static final class Adapter implements ContainerResponseWriter {
        ...
        public OutputStream writeResponseStatusAndHeaders(final long contentLength, final ContainerResponse response) {
            return crw.writeResponseStatusAndHeaders(contentLength, response);
        }
